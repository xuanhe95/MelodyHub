...
static async getAlbumCover(albumName: string): Promise<string> {
    const albumMid = await AlbumCoverAPI.getAlbumMbid(albumName)
    ...
    try {
        const response = await axios.get(url, { headers: AlbumCoverAPI.COMMON_HEADERS });
...
static async getAlbumMbid(query: string): Promise<string> {
